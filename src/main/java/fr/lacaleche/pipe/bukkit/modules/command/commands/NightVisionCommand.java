...
public class NightVisionCommand {
    ...
    public boolean execute(Command<CommandSender> command) {
        ...
        if (targets.size() == 1) {
            command.sender().sendMessage(command.locale().ct("pipe.command.nightvision.success.enabled", "pipe.command.nightvision.success.disabled", result.getPlayer().hasPotionEffect(PotionEffectType.NIGHT_VISION)).arg("player", result.getPlayer().getName()).from("Night Vision").ct());
        } else {
