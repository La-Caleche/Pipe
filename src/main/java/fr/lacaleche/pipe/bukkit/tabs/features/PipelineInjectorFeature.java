...
public class PipelineInjectorFeature extends AbstractTabFeature implements Loadable, Unloadable, JoinListener, Injector {
    ...
    private Channel getChannel(TabPlayer tabPlayer) {
        ...
        Object networkManager = this.storage().get(StorageFields.PLAYER_CONNECTION$NETWORK_MANAGER, connection);
        if (networkManager == null) return null;

        return this.storage().get(StorageFields.NETWORK_MANAGER$CHANNEL, networkManager);
    }
    ...
    public boolean isPlayerInfo(Object packet) {
        return this.storage().clazz(StorageClass.PACKET_CLIENTBOUND_PLAYER_INFO_UPDATE).isInstance(packet);
    }

    public void onPlayerInfo(TabPlayer receiver, Object packet) {
        List<String> actions = ((EnumSet<?>) this.storage().get(StorageFields.PCB_PLAYER_INFO_UPDATE$ACTIONS, packet)).stream().map(Enum::name).toList();
        List<Object> updatedList = new ArrayList<>();

        for (Object playerDataEntry : (List<?>) this.storage().get(StorageFields.PCB_PLAYER_INFO_UPDATE$PLAYERS, packet)) {
            GameProfile profile = (GameProfile) this.storage().invoke(StorageMethods.PCB_PLAYER_INFO_DATA$GET_PROFILE, playerDataEntry);
            int gameMode = 0;
            int latency = 0;
            Component displayName = null;
            boolean listed = this.storage().get(StorageFields.PCB_PLAYER_INFO_DATA$LISTED, playerDataEntry);
            Object chatSession = this.storage().get(StorageFields.PCB_PLAYER_INFO_DATA$CHAT_SESSION, playerDataEntry);

            if (actions.contains("UPDATE_GAME_MODE") || actions.contains("ADD_PLAYER")) {
                gameMode = this.tab().gameMode2Int(this.storage().get(StorageFields.PCB_PLAYER_INFO_DATA$GAME_MODE, playerDataEntry));
            ...
            if (actions.contains("UPDATE_LATENCY") || actions.contains("ADD_PLAYER")) {
                latency = this.storage().get(StorageFields.PCB_PLAYER_INFO_DATA$LATENCY, playerDataEntry);
            ...
            if (actions.contains("UPDATE_DISPLAY_NAME") || actions.contains("ADD_PLAYER")) {
                IChatBaseComponent displayNameComponent = this.storage().get(StorageFields.PCB_PLAYER_INFO_DATA$DISPLAY_NAME, playerDataEntry);
                displayName = displayNameComponent == null ? null : this.storage().invoke(StorageMethods.ADVENTURE_COMPONENT$GET_COMPONENT, this.storage().cast(StorageClass.ADVENTURE_COMPONENT, displayNameComponent));
                displayName = this.tab().onDisplayNameChange(receiver, profile.getId(), displayName);
            }

            updatedList.add(this.storage().construct(StorageConstructor.PCB_PLAYER_INFO_DATA_CONSTRUCTOR,
                    profile.getId(),
                    profile,
                    listed,
                    latency,
                    this.tab().int2GameMode(gameMode),
                    this.storage().construct(StorageConstructor.ADVENTURE_COMPONENT_CONSTRUCTOR, displayName),
                    chatSession
            ));
        }

        this.storage().set(StorageFields.PCB_PLAYER_INFO_UPDATE$PLAYERS, packet, updatedList);
    }
